...
#    ENV['facter_role']='cd_demo'
    it 'should work idempotently with no errors' do
      expect(shell('facter_role=cd_demo puppet apply -t /etc/puppet/manifests/site.pp').exit_code).to_not eq(1)
      expect(shell('facter_role=cd_demo puppet apply -t /etc/puppet/manifests/site.pp').exit_code).to eq(0)
#      expect(shell('puppet apply -t /etc/puppet/manifests/site.pp').exit_code).to_not eq(1)
#      expect(shell('puppet apply -t /etc/puppet/manifests/site.pp').exit_code).to eq(0)
    end
